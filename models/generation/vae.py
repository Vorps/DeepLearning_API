...
class VAE(network.Network):
    ...
    class AutoEncoderBlock(network.ModuleArgsDict):
        ...
        def __init__(self, channels: list[int], nb_conv_per_stage: int, blockConfig: blocks.BlockConfig, downSampleMode: blocks.DownSampleMode, upSampleMode: blocks.UpSampleMode, dim: int, i : int = 0) -> None:
            super().__init__()
            if i > 0:
            ...
            if len(channels) > 2:
                self.add_module("AutoEncoder_{}".format(i+1), VAE.AutoEncoderBlock(channels[1:], nb_conv_per_stage, blockConfig, downSampleMode, upSampleMode, dim, i+1))
                self.add_module("UpConvBlock", blocks.ConvBlock(in_channels=channels[2] if upSampleMode != blocks.UpSampleMode.CONV_TRANSPOSE else channels[1], out_channels=channels[1], nb_conv=nb_conv_per_stage, blockConfig=blockConfig, dim=dim))
            if i > 0:
    ...
    def __init__(self,
                    ...
                    upSampleMode: str = "CONV_TRANSPOSE") -> None:
        
        super().__init__(in_channels = channels[0], init_type="kaiming", optimizer = optimizer, schedulers = schedulers, outputsCriterions = outputsCriterions, dim=dim, nb_batch_per_step=1)
        
        self.add_module("AutoEncoder_0", VAE.AutoEncoderBlock(channels, nb_conv_per_stage, blockConfig, downSampleMode=blocks.DownSampleMode._member_map_[downSampleMode], upSampleMode=blocks.UpSampleMode._member_map_[upSampleMode], dim=dim))

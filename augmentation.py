...
class Flip(Scale):
    ...
    def _state_init(self, index : int, shapes: list[list[int]], caches_attribute: list[Attribute]) -> list[list[int]]:
        func = _scale3DMatrix if len(self.f_prob) == 3 else _scale2DMatrix
        flip = torch.where(torch.rand((len(shapes), len(self.f_prob))) < torch.tensor(self.f_prob), -1, 1)
        self.matrix[index] = [torch.unsqueeze(func(value), dim=0) for value in flip]
        return shapes

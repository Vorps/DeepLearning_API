...
class UNetBlock(network.ModuleArgsDict):
    ...
    def __init__(self, channels: List[int], blockConfig: blocks.BlockConfig, downSampleMode: blocks.DownSampleMode, upSampleMode: blocks.UpSampleMode, attention : bool, dim: int, i : int = 0) -> None:
        super().__init__()
        if i > 0:
        ...
        self.add_module("DownConvBlock", blocks.ConvBlock(in_channels=channels[1 if downSampleMode == blocks.DownSampleMode.CONV_STRIDE else 0], out_channels=channels[1], blockConfig=blockConfig, dim=dim))
        if len(channels) > 2:
            self.add_module("UNetBlock_{}".format(i+1), UNetBlock(channels[1:], blockConfig, downSampleMode, upSampleMode, attention, dim, i+1))
            self.add_module("UpConvBlock", blocks.ConvBlock(in_channels=(channels[1]+channels[2]) if upSampleMode != blocks.UpSampleMode.CONV_TRANSPOSE else channels[1]*2, out_channels=channels[1], blockConfig=blockConfig, dim=dim))
        if i > 0:
            if attention:

...
from DeepLearning_API import CONFIG_FILE, STATISTICS_DIRECTORY, PREDICTIONS_DIRECTORY, DL_API_STATE, CUDA_VISIBLE_DEVICES
...
class TensorBoard():
    ...
    def __enter__(self):
        if "DEEP_LEARNING_TENSORBOARD_PORT" in os.environ:
            command = ["tensorboard", "--logdir", self.path + self.name + "/", "--port", os.environ["DEEP_LEARNING_TENSORBOARD_PORT"], "--bind_all"]
...
class DistributedObject():
    ...
    def __call__(self, rank: Union[int, None] = None) -> None:
        with Log(self.name, PREDICTIONS_DIRECTORY() if DL_API_STATE() == "PREDICTION" else STATISTICS_DIRECTORY()) as log:
